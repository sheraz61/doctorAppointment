...
try {
    ...
    if (data.success){
        ...
        setExperience('General physician')
    }else{
        toast.error(data.message)
    }
} catch (error) {
    toast.log(error.messsage);
}
}
    ...
    return (
        <form onSubmit={onSubmitHandler} className='m-5 w-full'>
            ...
            <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
                ...
                <button type='submit' className='bg-[#5f5FFF] px-10 py-3 mt-4 text-white rounded-full cursor-pointer'>Add Doctor</button>
            </div>
        </form>
    )
}
